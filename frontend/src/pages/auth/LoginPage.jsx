...
import {
  ...
  AuthHeading, ErrorBanner, UnverifiedBanner, AuthFooter,
} from './_authShared';
import {
  loginUser,
  selectIsLoading,
...
const LoginPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (!result.error) {
      ...
      if (msg.includes('verify') || msg.includes('verification')) setUnverified(true);
    }
  };
  ...
  return (
    <AuthLayout>
      <AuthHeading title="Welcome back" subtitle="Sign in to your Ticket Bro account." />
      ...
      <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 8 }}>
          ...
          label={
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Link
                ...
                style={{ fontSize: 11.5, color: 'var(--muted-foreground,#9ca3af)', textDecoration: 'none', fontWeight: 400 }}
                onMouseEnter={e => (e.target.style.color = '#a3e635')}
                onMouseLeave={e => (e.target.style.color = 'var(--muted-foreground,#9ca3af)')}
              >
                Forgot password?
              </Link>
            </div>
        ...
        >
          <input className="af-input" {...register('password')} type={showPw ? 'text' : 'password'} placeholder="••••••••" autoComplete="current-password" />
        </Field>

        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', width: 'fit-content', userSelect: 'none' }}>
          <input {...register('rememberMe')} type="checkbox" style={{ width: 14, height: 14, accentColor: '#a3e635', cursor: 'pointer' }} />
          <span style={{ fontSize: 12.5, color: 'var(--muted-foreground,#6b7280)' }}>Remember me</span>
        </label>

        <SubmitBtn loading={isLoading}>Sign in <ArrowRight size={15} /></SubmitBtn>
      </form>

      <AuthFooter text="Don't have an account?" linkText="Create one →" to={ROUTES.AUTH.REGISTER} />
    </AuthLayout>
  );
};
