    ...
    <div className="flex items-center justify-center gap-1 pt-6">
      ...
      {deduped.map((p, i) => p === "..." ? (
        ...
        <button key={p} onClick={() => onPageChange(p)}
          ...
          style={{ background: p === page ? "var(--foreground)" : "var(--background)", borderColor: p === page ? "var(--foreground)" : "var(--border)", color: p === page ? "var(--background)" : "var(--muted-foreground)", fontFamily: "var(--font-sans)" }}>
          {p}
        </button>
      ))}
...
const EventGridSection = ({ filters }) => {
  const { getEvents, level, categorySlug, subCategorySlug, eventTypeSlug, locationLabel } = useBrowse();
  ...
  const pageEvents = allEvents.slice((page - 1) * EVENTS_PER_PAGE, page * EVENTS_PER_PAGE);
  ...
  const sectionTitle = level === "root" ? "All Events"
    : level === "category" ? `All ${unslugify(categorySlug)} Events`
    : level === "subCategory" ? `All ${unslugify(subCategorySlug)} Events`
    : `All ${unslugify(eventTypeSlug)} Events`;

  const levelLabel = level === "root" ? "" : level === "category" ? unslugify(categorySlug)
    : level === "subCategory" ? unslugify(subCategorySlug) : unslugify(eventTypeSlug);
      ...
      <Container>
        <div className="py-8">
          <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
            <div>
              <h2 className="text-xl font-bold text-foreground" style={{ fontFamily: "var(--font-heading)" }}>{sectionTitle}</h2>
            ...
            <div className="flex items-center rounded-md border border-border overflow-hidden shrink-0">
              {[["grid", LayoutGrid], ["list", List]].map(([mode, Icon]) => (
                <button key={mode} onClick={() => setViewMode(mode)}
                  ...
                  style={{ background: viewMode === mode ? "var(--foreground)" : "var(--background)", color: viewMode === mode ? "var(--background)" : "var(--muted-foreground)" }}
          ...
          {pageEvents.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {pageEvents.map((e) => <BrowseEventCard key={e.id} event={e} variant="grid" saved={savedIds.has(e.id)} onSave={toggle} />)}
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {pageEvents.map((e) => <BrowseEventCard key={e.id} event={e} variant="list" saved={savedIds.has(e.id)} onSave={toggle} />)}
            </div>
          )}
