...
import {
  MapPin, Navigation, Copy, ExternalLink, Wifi, Globe,
  Check, Info,
} from "lucide-react";
import { SectionHeading, InfoRow } from "../shared/EventShared";
...
const EventVenueSection = ({ event }) => {
  ...
  const address   = loc.addressLabel || [loc.name, loc.address, loc.city, loc.country].filter(Boolean).join(", ");
    ...
    <div className="flex flex-col gap-5">
      <SectionHeading>
        ...
        <span className="text-xs font-normal text-muted-foreground">
          {loc.typeLabel || (isOnline ? "Online" : isHybrid ? "Hybrid" : "In Person")}
        </span>
      </SectionHeading>

      {/* Online details */}
      {isOnline && (
        <div className="p-4 rounded-xl border border-border flex flex-col gap-3"
          ...
          {loc.onlineUrl && (
            <a href={loc.onlineUrl} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-xs font-semibold text-foreground hover:underline"
              style={{ fontFamily: "var(--font-sans)" }}>
              <Globe size={12} />Join Event Link <ExternalLink size={11} />
            </a>
          )}
        </div>
      )}
      ...
      {!isOnline && (
        <>
          <div className="rounded-xl overflow-hidden border border-border bg-muted" style={{ height: 240 }}>
            <iframe
              title="Event venue"
              src={embedUrl}
          ...
          <div className="flex flex-col gap-3 p-4 rounded-xl border border-border"
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button onClick={copyAddress}
                className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border border-border hover:bg-accent transition-colors"
                style={{ fontFamily: "var(--font-sans)" }}>
                {copied ? <><Check size={11} className="text-foreground" /> Copied!</> : <><Copy size={11} /> Copy Address</>}
              </button>
              <a href={mapsUrl} target="_blank" rel="noopener noreferrer"
