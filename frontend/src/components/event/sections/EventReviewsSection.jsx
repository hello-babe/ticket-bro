...
import { SectionHeading, StarRow, AvatarCircle, timeAgo } from "../shared/EventShared";

const MOCK_REVIEWS = [
  { id: 1, author: "Rafiq Ahmed",   initial: "R", averageRating: 5, title: "Best concert I've attended!",
    ...
    helpful: 47, verified: true,  createdAt: "2025-03-01T10:00:00Z" },
  { id: 2, author: "Nadia Khan",    initial: "N", averageRating: 5, title: "Unforgettable night",
    ...
    helpful: 34, verified: true,  createdAt: "2025-02-28T14:30:00Z" },
  { id: 3, author: "Imran Hossain", initial: "I", averageRating: 4, title: "Great event, minor issues",
    ...
    helpful: 21, verified: false, createdAt: "2025-02-26T19:00:00Z" },
  { id: 4, author: "Sadia Islam",   initial: "S", averageRating: 5, title: "Pure rock bliss",
    ...
    helpful: 18, verified: true,  createdAt: "2025-02-25T08:00:00Z" },
  { id: 5, author: "Tanvir Alam",   initial: "T", averageRating: 4, title: "Loved it",
    body: "Great lineup, great crowd. A few technical hiccups early on but they sorted it quickly. Overall a massive success.",
    helpful: 12, verified: false, createdAt: "2025-02-24T12:00:00Z" },
];
...
const EventReviewsSection = ({ event }) => {
  ...
  const toggle = (id) => setHelpfulSet((p) => {
    const n = new Set(p);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Rating summary */}
      <div className="flex gap-6 p-5 rounded-2xl border border-border"
        style={{ background: "var(--card)" }}>
        {/* Big number */}
        <div className="flex flex-col items-center justify-center shrink-0 gap-1">
          <p className="text-5xl font-extrabold text-foreground"
            style={{ fontFamily: "var(--font-heading)" }}>{rating.toFixed(1)}</p>
        ...
        <div className="flex-1 flex flex-col gap-1.5 justify-center">
          {RATING_DIST.map(({ stars, pct }) => (
            <div key={stars} className="flex items-center gap-2">
              <span className="text-[11px] font-medium text-foreground w-2 shrink-0"
                style={{ fontFamily: "var(--font-sans)" }}>{stars}</span>
              <Star size={9} className="text-foreground fill-foreground shrink-0" />
              <div className="flex-1 h-1.5 rounded-full bg-border overflow-hidden">
                <div className="h-full rounded-full transition-all"
                  style={{ width: `${pct}%`, background: "var(--foreground)" }} />
              </div>
              <span className="text-[11px] text-muted-foreground w-7 text-right shrink-0"
                style={{ fontFamily: "var(--font-sans)" }}>{pct}%</span>
            </div>
          ))}
        </div>
      </div>
                ...
                <div>
                  <div className="flex items-center gap-1">
                    <p className="text-xs font-semibold text-foreground"
                      style={{ fontFamily: "var(--font-sans)" }}>{r.author}</p>
                    {r.verified && <BadgeCheck size={11} className="text-foreground" />}
                  </div>
                  <p className="text-[10px] text-muted-foreground"
                    style={{ fontFamily: "var(--font-sans)" }}>{timeAgo(r.createdAt)}</p>
                </div>
              </div>
              <StarRow rating={r.averageRating} size={11} />
            </div>
            <p className="text-xs font-bold text-foreground mb-1"
              style={{ fontFamily: "var(--font-heading)" }}>{r.title}</p>
            <p className="text-xs text-muted-foreground leading-relaxed"
              style={{ fontFamily: "var(--font-sans)" }}>{r.body}</p>
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
              <span className="text-[10px] text-muted-foreground"
                style={{ fontFamily: "var(--font-sans)" }}>Helpful?</span>
              <button onClick={() => toggle(r.id)}
                className="flex items-center gap-1.5 text-[11px] transition-colors"
                style={{
                  color: helpfulSet.has(r.id) ? "var(--foreground)" : "var(--muted-foreground)",
                  fontFamily: "var(--font-sans)",
                }}>
                <ThumbsUp size={11} className={helpfulSet.has(r.id) ? "fill-foreground" : ""} />
      ...
      {reviews.length > 3 && (
        <button onClick={() => setShowAll((p) => !p)}
          ...
          {showAll ? "Show fewer reviews" : `View all ${total.toLocaleString()} reviews`}
        </button>
      )}
    </div>
  );
