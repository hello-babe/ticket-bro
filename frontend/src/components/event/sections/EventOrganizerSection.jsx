...
import {
  BadgeCheck, Globe, Mail, Phone,
  Instagram, Facebook, Twitter, Youtube,
  ChevronRight, Star, Calendar, Users,
} from "lucide-react";
import { SectionHeading, AvatarCircle, StarRow } from "../shared/EventShared";
...
const EventOrganizerSection = ({ event }) => {
  ...
  const stats = [
    ...
    { icon: Users,    label: "Attendees", value: (org.totalAttendees || 0).toLocaleString() },
    { icon: Star,     label: "Rating",    value: (org.averageRating || org.rating || 0).toFixed(1) },
  ];
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="p-5 rounded-2xl border border-border flex flex-col gap-4"
        ...
        <div className="flex items-start gap-3">
          {org.avatar
            ? <img src={org.avatar} alt={name} className="w-12 h-12 rounded-full object-cover border border-border" />
            : <AvatarCircle initial={avatar} size={3} className="!text-[18px] !w-12 !h-12" />
          }
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              <p className="text-base font-bold text-foreground"
                style={{ fontFamily: "var(--font-heading)" }}>{name}</p>
              {(org.isVerified || org.verified) && <BadgeCheck size={15} className="text-foreground" />}
            </div>
