...
import {
  ...
  Share2, Flag, BadgeCheck, Star, ZoomIn, Play,
  Calendar, Clock, MapPin, Users, Flame, Sparkles,
} from "lucide-react";
import Container from "@/components/layout/Container";
import {
  StarRow, TagPill, InfoRow, CapacityBar, AvatarCircle,
  PriceBadge, StatusBadge, fmtDate, fmtTime, fmtNum,
} from "../shared/EventShared";

const EventHeroSection = ({ event, saved, onSave, onShare, onBook }) => {
  const [activeImg, setActiveImg] = useState(0);
  const [lightbox, setLightbox]   = useState(false);

  const images = event.images?.length ? event.images : [event.coverImage].filter(Boolean);
  const prev = useCallback(() => setActiveImg((p) => (p - 1 + images.length) % images.length), [images.length]);
  const next = useCallback(() => setActiveImg((p) => (p + 1) % images.length), [images.length]);
  ...
  return (
    <>
      ...
      <div className="relative w-full overflow-hidden" style={{ height: "clamp(300px, 50vh, 560px)" }}>
        ...
        <div className="absolute top-0 left-0 right-0 flex items-start justify-between p-4 sm:p-6">
          <div className="flex gap-1.5 flex-wrap">
            ...
            {event.isFeatured && (
              <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border"
                style={{ background: "rgba(163,230,53,0.15)", borderColor: "rgba(163,230,53,0.4)", color: "#a3e635" }}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setLightbox(true)} aria-label="Zoom"
              className="w-9 h-9 rounded-full flex items-center justify-center border border-white/20 bg-black/40 backdrop-blur-sm hover:bg-black/60 transition-all">
            ...
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white leading-tight mb-1"
                  ...
                  {event.title}
                  {event.isVerified && <BadgeCheck size={22} className="inline ml-2 text-lime-400" />}
                </h1>
                {/* Rating */}
                <div className="flex items-center gap-2">
                  <StarRow rating={event.averageRating} size={13} />
                  <span className="text-sm font-bold text-white">{(event.averageRating || 0).toFixed(1)}</span>
                  <span className="text-sm text-white/70">({fmtNum(event.reviewCount)} reviews)</span>
                </div>
              </div>
              {/* Image counter */}
              {images.length > 1 && (
                <div className="flex gap-1 shrink-0">
                  {images.map((_, i) => (
                    <button key={i} onClick={() => setActiveImg(i)}
                      ...
                      style={{
                        width: i === activeImg ? 20 : 6, height: 6,
                        background: i === activeImg ? "var(--color-brand-primary, #a3e635)" : "rgba(255,255,255,0.4)",
                      }}
                    />
                  ))}
                </div>
        ...
        <Container>
          <div className="py-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center border border-border shrink-0"
                style={{ background: "var(--secondary)" }}>
                <Calendar size={15} className="text-foreground" />
              </div>
              <div>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide" style={{ fontFamily: "var(--font-sans)" }}>Date</p>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide" style={{ fontFamily: "var(--font-sans)" }}>Time</p>
                <p className="text-xs font-semibold text-foreground leading-tight" style={{ fontFamily: "var(--font-sans)" }}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide" style={{ fontFamily: "var(--font-sans)" }}>Venue</p>
                <p className="text-xs font-semibold text-foreground leading-tight truncate" style={{ fontFamily: "var(--font-sans)" }}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide" style={{ fontFamily: "var(--font-sans)" }}>Attending</p>
                <p className="text-xs font-semibold text-foreground leading-tight" style={{ fontFamily: "var(--font-sans)" }}>
