...
import { Ticket, Check, Lock, ChevronRight, Shield, Zap, Clock } from "lucide-react";
import { CapacityBar, PriceBadge } from "../shared/EventShared";
...
const EventTicketsSection = forwardRef(({ event }, ref) => {
  ...
  const [selected, setSelected] = useState(available[1]?.id || available[0]?.id);
  ...
  return (
    <div ref={ref} className="flex flex-col gap-5">
      ...
      <div>
        ...
        <p className="text-[10px] text-muted-foreground mt-1" style={{ fontFamily: "var(--font-sans)" }}>
          {soldPct}% sold
          ...
          return (
            <button key={t.id}
              ...
              style={{
                ...
                boxShadow:   isSelected ? "inset 0 0 0 1px var(--foreground)" : "none",
              }}>
              ...
              <span className="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5"
                ...
                {isSelected && <span className="w-2 h-2 rounded-full" style={{ background: "var(--foreground)" }} />}
              </span>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-bold text-foreground" style={{ fontFamily: "var(--font-heading)" }}>
                    {t.label}
                    {isSoldOut && (
                      <span className="ml-2 text-[10px] font-normal text-muted-foreground">(Sold out)</span>
                    )}
                  </span>
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                  {(t.perks || []).map((p, i) => (
                    <span key={i} className="flex items-center gap-1 text-[11px] text-muted-foreground"
                      style={{ fontFamily: "var(--font-sans)" }}>
                      <Check size={9} className="text-foreground" />{p}
                    </span>
                  ))}
                </div>
              </div>
        ...
        <div className="flex items-center justify-between gap-4 p-4 rounded-xl border border-border"
          ...
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1"
              style={{ fontFamily: "var(--font-sans)" }}>Quantity</p>
          ...
          <div className="text-right">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide"
              style={{ fontFamily: "var(--font-sans)" }}>Total</p>
      ...
      <button
        ...
        <Ticket size={15} />
        {!canPurchase ? "Not Available" : `Book Now · ৳${total.toLocaleString()}`}
      </button>
      ...
      <div className="flex items-center justify-center gap-4 flex-wrap">
        {[
          ...
          <span key={text} className="flex items-center gap-1 text-[10px] text-muted-foreground"
            style={{ fontFamily: "var(--font-sans)" }}>
            <Icon size={10} />{text}
          </span>
        ))}
