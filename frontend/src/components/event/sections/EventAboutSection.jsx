...
import {
  Check, ChevronDown, ChevronUp, Shield, Accessibility,
  Shirt, AlertCircle, RotateCcw,
} from "lucide-react";
import { SectionHeading, EDivider, TagPill } from "../shared/EventShared";
    ...
    <div className="flex flex-col gap-6">
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {tags.map((t, i) => <TagPill key={i}>{t.name || t}</TagPill>)}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {/* Age */}
        <div className="flex flex-col gap-1 p-3 rounded-lg border border-border"
          style={{ background: "var(--secondary)" }}>
          <AlertCircle size={14} className="text-muted-foreground" />
          <p className="text-[10px] text-muted-foreground uppercase tracking-wide mt-1"
            style={{ fontFamily: "var(--font-sans)" }}>Age</p>
          <p className="text-xs font-semibold text-foreground"
            style={{ fontFamily: "var(--font-sans)" }}>{ageLabel}</p>
        </div>
        {/* Dress code */}
        <div className="flex flex-col gap-1 p-3 rounded-lg border border-border"
          style={{ background: "var(--secondary)" }}>
          ...
          <p className="text-[10px] text-muted-foreground uppercase tracking-wide mt-1"
            style={{ fontFamily: "var(--font-sans)" }}>Dress Code</p>
          <p className="text-xs font-semibold text-foreground"
            style={{ fontFamily: "var(--font-sans)" }}>{dressCode || "Casual"}</p>
        </div>
        {/* Refund */}
        <div className="flex flex-col gap-1 p-3 rounded-lg border border-border"
          style={{ background: "var(--secondary)" }}>
          ...
          <p className="text-[10px] text-muted-foreground uppercase tracking-wide mt-1"
            style={{ fontFamily: "var(--font-sans)" }}>Refunds</p>
          <p className="text-xs font-semibold text-foreground"
            style={{ fontFamily: "var(--font-sans)" }}>
            {refund?.allowRefunds
        ...
        <div className="flex flex-col gap-1 p-3 rounded-lg border border-border"
          ...
          <p className="text-[10px] text-muted-foreground uppercase tracking-wide mt-1"
            style={{ fontFamily: "var(--font-sans)" }}>Accessibility</p>
          <p className="text-xs font-semibold text-foreground"
            style={{ fontFamily: "var(--font-sans)" }}>
