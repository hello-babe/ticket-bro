...
const TIER_LABEL = { platinum: "Platinum", gold: "Gold", silver: "Silver", bronze: "Bronze", partner: "Partners" };
          ...
          <div className="flex flex-wrap gap-3">
            {list.map((s, i) => (
              <a key={s._id || i}
                ...
                {s.logo
                  ? <img src={s.logo} alt={s.name} className="h-6 object-contain max-w-[80px]" />
                  : <span className="text-sm font-bold text-foreground" style={{ fontFamily: "var(--font-heading)" }}>{s.name}</span>
                }
