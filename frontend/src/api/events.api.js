...
export const getAllEvents = (params) =>
  api.get("/events", { params }).then((r) => r.data.data?.events || []);
export const getFeaturedEvents = (limit = 6) =>
  api
    .get("/events/featured", { params: { limit } })
    .then((r) => r.data.data?.events || []);
export const getTrendingEvents = (limit = 6) =>
  api
    .get("/events/trending", { params: { limit } })
    .then((r) => r.data.data?.events || []);
export const getUpcomingEvents = (limit = 8) =>
  api
    .get("/events/upcoming", { params: { limit } })
    .then((r) => r.data.data?.events || []);
...
export const getCategoryData = async (slug) => {
  ...
  return {
    ...
    stats: {
      ...
      averageRating:
        events.reduce((s, e) => s + (e.rating || 0), 0) / (events.length || 1),
...
export const getSubcategoryData = async (cat, sub) => {
  const [category, subcategory, events] = await Promise.all([
    ...
    api
      .get(`/categories/${cat}/subcategories`)
      .then((r) =>
        (r.data.data?.subcategories || []).find((s) => s.slug === sub),
      ),
...
export const searchEvents = (query, filters = {}) =>
  api
    .get("/search", { params: { q: query, ...filters } })
...
export const cancelEvent = (slug, data) =>
  api.post(`/events/${slug}/cancel`, data);
export const createTicketType = (slug, data) =>
  api.post(`/events/${slug}/ticket-types`, data);
export const updateTicketType = (slug, id, d) =>
  api.put(`/events/${slug}/ticket-types/${id}`, d);
export const deleteTicketType = (slug, id) =>
  api.delete(`/events/${slug}/ticket-types/${id}`);
export const createSeatSection = (slug, data) =>
  api.post(`/events/${slug}/seat-sections`, data);
export const updateSeatSection = (slug, id, d) =>
  api.put(`/events/${slug}/seat-sections/${id}`, d);
